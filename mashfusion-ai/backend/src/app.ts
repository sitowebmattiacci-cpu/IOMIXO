...
app.use(cors({
  ...
  allowedHeaders: ['Content-Type', 'Authorization', 'X-Request-ID', 'X-IOMIXO-Device'],
}))
...
app.get('/health', (_, res) => res.json({ status: 'ok', build: 'event-pass-coupon-fix-2026-06-12', ts: new Date().toISOString() }))
